// --- BROWSER TESTING MOCK API ---
if (typeof window.pywebview === 'undefined') {
  window.pywebview = {
    api: {
      _db: null,
      _loadDb: async function () {
        if (this._db) return this._db;
        try {
          const res = await fetch('database.json');
          this._db = await res.json();
        } catch (e) {
          this._db = { leaderboard: [], posts: [], fakta: [] };
        }
        const local = localStorage.getItem('mock_db');
        if (local) {
          this._db = JSON.parse(local);
        }
        return this._db;
      },
      _saveDb: function () {
        localStorage.setItem('mock_db', JSON.stringify(this._db));
      },
      getGameData: async function (hari) {
        const db = await this._loadDb();
        const posts = db.posts.filter(p => p.hari === hari).map(p => ({
          author: p.author || "",
          text: p.text || "",
          imgUrl: p.imgUrl || "",
          isAiImage: !!p.isAiImage,
          isHoax: !!p.isHoax,
          tanggal: p.tanggal || ""
        }));
        const fakta = db.fakta.filter(f => f.hari === hari).map(f => ({
          judul: f.judul || "",
          isi: f.isi || ""
        }));
        return { posts, fakta };
      },
      simpanProgres: async function (nama, skor, level) {
        const db = await this._loadDb();
      ...
      muatProgres: async function (nama) {
        const db = await this._loadDb();
        const leaderboard = db.leaderboard || [];
        for (let i = leaderboard.length - 1; i >= 0; i--) {
          const entry = leaderboard[i];
          if (String(entry.Nama_Pemain).toLowerCase() === String(nama).toLowerCase()) {
            return {
              ditemukan: true,
              skor: entry.Skor_Akhir,
              level: entry.Level_Hari
            };
          }
        }
        return { ditemukan: false };
      },
      ambilDaftarSimpanan: async function () {
        const db = await this._loadDb();
        const leaderboard = db.leaderboard || [];
        const simpananTerbaru = {};
        leaderboard.forEach(entry => {
          const nama = String(entry.Nama_Pemain);
          simpananTerbaru[nama] = {
            nama: nama,
            skor: entry.Skor_Akhir,
            level: entry.Level_Hari
          };
        });
        return Object.values(simpananTerbaru).filter(d => d.level < 4);
      },
      ambilLeaderboard: async function () {
        const db = await this._loadDb();
        const leaderboard = db.leaderboard || [];
        const simpananTerbaru = {};
        leaderboard.forEach(entry => {
          const nama = String(entry.Nama_Pemain);
          const skor = entry.Skor_Akhir || 0;
          if (!simpananTerbaru[nama] || skor > simpananTerbaru[nama].skor) {
            simpananTerbaru[nama] = { nama, skor };
          }
        });
        return Object.values(simpananTerbaru).sort((a, b) => b.skor - a.skor);
      }
    }
  };
}
...
const GameController = {
  ...
  tampilkanBossMarah: function (pesan) {
    const overlay = document.getElementById('boss-sp-overlay');
    const bossImg = document.getElementById('boss-sp-img');
    const bossText = document.getElementById('boss-sp-text');

    overlay.style.display = 'block';
    bossText.innerText = pesan;

    bossImg.style.bottom = '-152px';
    bossImg.style.right = '-192px';
    bossText.style.opacity = '0';

    setTimeout(() => {
      bossImg.style.bottom = '0px';
      bossImg.style.right = '0px';
  ...
  pindahView: function (viewId) {
    // Jika game sudah selesai/game over, jangan biarkan pindah kembali ke room/komputer
    if ((viewId === 'room' || viewId === 'komputer') &&
      (document.getElementById('level-end').classList.contains('active') ||
    ...
    if (viewId === 'tutup-bukusaku') {
      document.getElementById('bukusaku').classList.remove('active');
      return;
    }
  ...
  lanjutkanSif: function () {
    ...
    const kalenderImg = document.getElementById('kalender-hari');
    if (kalenderImg) {
      if (this.levelHariIni === 1) {
        kalenderImg.src = 'asset/img/hari/hari1_42x40.png';
        kalenderImg.style.width = '42px';
        kalenderImg.style.height = '40px';
      } else if (this.levelHariIni === 2) {
        kalenderImg.src = 'asset/img/hari/hari2_44x27.png';
        kalenderImg.style.width = '44px';
        kalenderImg.style.height = '27px';
      } else if (this.levelHariIni === 3) {
        kalenderImg.src = 'asset/img/hari/hari3_44x27.png';
        kalenderImg.style.width = '44px';
        kalenderImg.style.height = '27px';
      } else if (this.levelHariIni === 4) {
        kalenderImg.src = 'asset/img/hari/hari4_44x27.png';
        kalenderImg.style.width = '44px';
        kalenderImg.style.height = '27px';
      }
    }
  ...
  loadPost: function () {
    ...
    const kompImgContainer = document.getElementById('komp-img-container');
    const btnScan = document.getElementById('btn-scan-action');

    // Reset scan timeouts
    if (this.scanTimeouts) {
    ...
    const scanIcon = document.getElementById('scan-result-icon');
    if (scanResultContainer) scanResultContainer.style.display = 'none';
    if (scanStatus) scanStatus.innerHTML = 'Menganalisis...';
    if (scanLine) scanLine.style.display = 'none';
    if (scanOverlay) scanOverlay.style.display = 'none';
    if (scanIcon) scanIcon.style.display = 'none';

    // Re-enable decision buttons
    const btnHapus = document.getElementById('btn-hapus-action');
    const btnLolos = document.getElementById('btn-lolos-action');
    if (btnHapus) btnHapus.classList.remove('disabled-button');
    if (btnLolos) btnLolos.classList.remove('disabled-button');
    if (btnScan) {
      btnScan.classList.remove('disabled-button');
      btnScan.innerText = "SCAN FOTO";
    }

    if (this.levelHariIni >= 3) {
      // Deterministically pick a local mock image if post.imgUrl is empty
      let imgUrl = post.imgUrl;
      if (!imgUrl || imgUrl === "") {
        const index = this.currentIndex;
        if (post.isAiImage) {
          imgUrl = (index % 2 === 0) ? "asset/img/pic_ai_1.png" : "asset/img/pic_ai_2.png";
        } else {
          imgUrl = (index % 2 === 0) ? "asset/img/pic_ori_1.png" : "asset/img/pic_ori_2.png";
        }
      } else {
        imgUrl = "asset/img/postingan/" + imgUrl + ".png";
      }

      roomImgEl.src = imgUrl;
      roomImgEl.style.display = "none"; // Set ke "none" agar gambar tidak muncul di monitor mini (room view)
      kompImgEl.src = imgUrl;
      if (kompImgContainer) kompImgContainer.style.display = "block";
      if (btnScan) btnScan.style.display = "inline-block";
    } else {
      roomImgEl.style.display = "none";
      if (kompImgContainer) kompImgContainer.style.display = "none";
  ...
  mulaiScan: function () {
    ...
    if (btnScan) btnScan.classList.add('disabled-button');
    if (btnHapus) btnHapus.classList.add('disabled-button');
    if (btnLolos) btnLolos.classList.add('disabled-button');

    // Show scanner visual effects
    if (scanLine) scanLine.style.display = 'block';
    if (scanOverlay) scanOverlay.style.display = 'block';
    if (scanResultContainer) scanResultContainer.style.display = 'block';

    // Status sequence
    if (scanStatus) scanStatus.innerHTML = '<span style="color: #f1c40f;">🔍 Menganalisis gambar</span>';

    this.scanTimeouts.push(setTimeout(() => {
      if (scanStatus) scanStatus.innerHTML = '<span style="color: #3498db;">⚙️ Memeriksa metadata</span>';
    }, 800));

    this.scanTimeouts.push(setTimeout(() => {
      if (scanStatus) scanStatus.innerHTML = '<span style="color: #e67e22;">⚠️ Mendeteksi artifak AI</span>';
    }, 1600));

    this.scanTimeouts.push(setTimeout(() => {
      // Hide scanner visual effects
      if (scanLine) scanLine.style.display = 'none';
      if (scanOverlay) scanOverlay.style.display = 'none';
      ...
      if (scanStatus) {
        if (post.isAiImage) {
          scanStatus.innerHTML = '<span style="color: #e74c3c; font-size: 10px;">🔴 HASIL SCAN: AI</span>';
          if (scanIcon) {
            scanIcon.src = 'asset/img/silang_12x12.png';
            scanIcon.style.display = 'block';
          }
        } else {
          scanStatus.innerHTML = '<span style="color: #2ecc71; font-size: 10px;">🟢 HASIL SCAN: SAFE</span>';
          if (scanIcon) {
            scanIcon.src = 'asset/img/centang_12x12.png';
            scanIcon.style.display = 'block';
          }
        }
      }

      if (btnScan) btnScan.innerText = "SELESAI SCAN";
    }, 2400));
  },
  ...
  eksekusi: function (keputusan) {
    ...
    if (salah) {
      if (this.salahLevelIni === 1) {
        this.tampilkanBossMarah("SURAT PERINGATAN 1: Anda mengambil keputusan yang keliru. Hati-hati!");
      } else if (this.salahLevelIni === 2) {
        this.tampilkanBossMarah("SURAT PERINGATAN 2: Ini peringatan terakhir! Satu kesalahan lagi Anda akan dipecat!");
